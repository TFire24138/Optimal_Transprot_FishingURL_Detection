...
for epoch in model_epochs:
    # 加载模型
    model.load_state_dict(torch.load(f"10_lnn_semi_epoch_{epoch}.pth", map_location=torch.device('cpu'), weights_only=True))
    model.eval()
    with torch.no_grad():
        # 进行预测
...
for epoch in model_epochs:
    if epoch not in best_epochs:
        os.remove(f"10_lnn_semi_epoch_{epoch}.pth")
