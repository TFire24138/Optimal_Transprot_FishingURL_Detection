...
import ot
from utils import ot_plan

# 确定设备，优先使用 GPU，如果 GPU 不可用则使用 CPU
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

def optimal_transport_pseudo_labels(model, labeled_loader, unlabeled_loader,logit_scale=1.0):
    """注释"""
    """
        使用最优传输生成无标签数据的伪标签
        :param model: 训练好的模型
        :param labeled_loader: 有标签数据的DataLoader
        :param unlabeled_loader: 无标签数据的DataLoader
        :return: 无标签数据的特征和伪标签
    """
    model.eval()
    ...
    """有标签数据的特征嵌入"""
    with torch.no_grad():
        for data, label in labeled_loader:
    ...
    """改进后的版本"""
    plan = ot_plan(unlabeled_features, labeled_features, logit_scale)
    # 基于传输计划生成伪标签
    # 从计划中选择匹配的标签
    pseudo_labels = torch.argmax(plan, dim=1)  # 选择匹配最大概率的标签作为伪标签
    return unlabeled_features, labeled_labels[pseudo_labels]
